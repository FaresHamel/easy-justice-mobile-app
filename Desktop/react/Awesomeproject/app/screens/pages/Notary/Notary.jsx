...
const Notary = ({navigation}) => {
  const [lawyers, setLawyers] = useState([]);
  const [loader, setLoader] = useState(false);
  const [error, setError] = useState(false);
  const fetchData = () => {
    setLoader(true);
    fetch('http://192.168.43.54:3000/notary')
        ...
        {loader ? (
          ...
          <View
            ...
            <Text style={{fontSize: 14, fontWeight: '700'}}>
              الرجاء اعادة المحاولة مرة اخرى
            </Text>
          </View>
        ) : judiciel.length === 0 ? (
          <Text>
            لايوجد موثقين متوفرين بعد سوف نعرض جميع البيانات هنا فورى توفرها في
            التطبيق
          </Text>
        ) : (
          lawyers.map(item => (
