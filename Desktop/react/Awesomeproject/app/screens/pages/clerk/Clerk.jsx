...
const Clerk = ({navigation}) => {
  ...
  const fetchData = () => {
    setLoader(true);
    fetch('http://192.168.43.54:3000/clerk')
      .then(response => response.json())
      .then(responseJson => {
        setLoader(false);
        setError(false)
        setClerk(responseJson);
      })
      .catch(error => {
        setLoader(false);
        setError(true)
    ...
    <SafeAreaView style={{backgroundColor: 'white', flex: 1}}>
      ...
      <ScrollView
        ...
        {loader ? (
          ...
          <Text>لا يوجد كاتب عمومي  بعد سوف نعرض جميع البيانات هنا فورى توفرها في التطبيق</Text>
            ) : clerk.length > 0(
              clerk.map(item => (
            <View
